...
## this returns 'currentVersion=0.31.0', will be cleaned up afterwards
VERSION_FROM_PROPERTIES=$(grep "^currentVersion" gradle.properties)
...
# patch-level bump: from 0.31.0 to 0.31.1
NEXT_VERSION=$(echo ${VERSION} | awk -F. -v OFS=. '$NF=sprintf("%0*d", length($NF+1), ($NF+1)%(10^length($NF+1)))')

echo "Releasing ${VERSION}"

## replace the version in the version.properties
sed "s/^currentVersion\=.*/currentVersion=${VERSION}/gi" -i version.properties
...
## publishing to bintray will happen on a tag build

## start next version
sed "s/^currentVersion\=.*/currentVersion=${NEXT_VERSION}/gi" -i version.properties
git add gradle.properties
git commit -qm "Next development version ${NEXT_VERSION}" --author="Jaeger Release <jaeger-release@jaegertracing.io>"
git push origin master ## this will fail if master has moved on
